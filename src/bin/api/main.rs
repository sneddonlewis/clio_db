...
pub fn jwt_decode(token: &str, jwk: &Jwk) -> ClioResult<Claims> {
...
pub fn encode_token(username: String) -> String {
    jwt_encode(username.as_str(), KEY_PRIV.as_bytes()).unwrap()
}

fn jwt_encode(username: &str, private_key: &[u8]) -> ClioResult<String> {
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let Some(jwks) = parts.extensions.get::<Jwks>() else {
